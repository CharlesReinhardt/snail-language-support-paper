...
\begin{wrapfigure}{r}{0.3\textwidth}
    \begin{center}
        \includegraphics[width=0.3\textwidth]{language-server.png}
...
The Langauge Server Protocol (LSP) defines a standard framework for development tool and language server communication \cite{Microsoft_2022a}. This way, a development tool simply sends and recieves information according to the LSP standard, and is able to communicate with any language-specific language server this way. Similarly, a langauge server will be able to communicate with any development tool by communicating via the LSP standard. 
...
VS Code's LSP relies on JSON-RPC requests and responses \cite{Microsoft_2023i}. A request is a message that a development tool sends to a language server. This message contains an identifier, method to be executed, and paramaters to be provided to that method. In order to follow the LSP, a language server \textbf{must} send the corresponding response message upon recieving any requests. This response message contains the request identifier, the result of the executed method, and an error object if the request fails. For example, a development tool might send the \lstinline{Initialize} request to a language server, and a language server must respond with the \lstinline{InitializeResponse} before the development tool is allowed to send any further requests. Language servers are also allowed to send requests to development tools, and development tools are still required to respond with the corresponding response.

VS Code's LSP also contains notifications. A notification is a message that does not require a response. A development tool can send a notification to a language server and the language server can process the notification as it sees fit. Often, the language server will send information back to the development tool (in the form of another notification), but it is important to note that the language server is not required to respond. 

With an established standard for communication, both development tools and language servers can streamline their communications systems. Additionally, a developer producing one of these tools or features can rely on a consistent standard to make the development process easier. This benefit is also illustrated in Figure \ref{fig:lsp-theory}.
...
\begin{figure}
    \begin{subfigure}{0.45\textwidth}
        \begin{center}
            \includegraphics[width=\textwidth]{dev-no-diagnostics.png}
            \subcaption{Developing snail code without realtime error diagnostics can make it difficult to spot syntax errors.}
    ...
    \begin{subfigure}{0.45\textwidth}
        \begin{center}
            \includegraphics[width=\textwidth]{dev-with-diagnostics.png}
            \subcaption{With Snail Language Support, a developer can spot easily spot syntax errors and fix them before runtime.}
            \label{fig:dev-with-diagnostics}
        \end{center}
    \end{subfigure}
    \caption{Demonstrating the benefit of realtime error diagnostics provided in Snail Language Support.}
...
\begin{wrapfigure}{C}{0.3\textwidth}
    \includegraphics[width=0.3\textwidth]{lsp-sequence.png}
    \caption{Modelling the interactions between a developer, a VS Code client, and snail language server while a developer opens and modifies a snail file.}
    \label{fig:lsp-sequence}
\end{wrapfigure}

A high-level sequence of interactions between a developer, VS Code, and the snail language server is documented in Figure \ref{fig:lsp-sequence}. First, a user opens a program written in snail (1). When VS Code detects this activity, it launches the snail language server (2). Next, VS Code provides the snail program for the language server to verify (3) and the language server executes the snail program, using the aforementioned \lstinline{snail -s} option (4). The snail language server returns the error diagnostics to VS Code (5), and VS code displays those diagnostics to the developer (6). The developer further modifies the snail file, and VS Code and the snail language server continue to communicate via LSP (7-10). When a developer is finished, they can close the snail file (11), and the VS Code client shuts down the snail language server (12). 
