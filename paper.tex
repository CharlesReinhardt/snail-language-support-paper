...
Using documentation formed a huge role in the development of the Snail Language Support extension. We used the VS Code documentation for starter guides on how to build VS Code extensions. The documentation even had numerous sample extensions for various general exstension purposes that were useful to build on top of. Documentation also contained granular details on various options and specific functionality of the extension API. Similar documentation also exists for the language server protocol (LSP) and debug adapter protocol (DAP). Some of this documentation is hosted on VS Code's documentation website, but most is hosted on a separate website. 

During development, Snail Language Support developers also made use of git documentation to look up specific syntax of commands. Some of this documentation is available while using git command line tools by passing the \lstinline{--help} flag.

Snail Language Support also creates documentation for the extension. One of the central goals of Snail Language Support development was to prioritize creating documentation during development. To do this, Snail Language Support leverages a few different documentation tools: Markdown, mermaidJS, and drawio. 

Markdown is a plain text syntax that takes lightly marked-up text and converts it to clean HTML \cite{Gruber_2004}. The syntax is designed to be lightweight, allowing text files written with Markdown syntax to still be fully legible without converting to HTML. Since it's birth in 2004, Markdown has seen widespread, mainstream adoption across the industry. Popular tools like GitHub and VS Code offer built-in support for Markdown syntax \cite{GitHub_2023, Microsoft_2023k}. Markdown syntax has also strongly influenced the R Markdown syntax for R programming \cite{RStudio_2020}. 

Snail Language Support uses Markdown to create a \lstinline{README.md} which describes valuable information about the extension for users and developers. Snail Language Support developers also used Markdown files as notepads to write down thoughts and discoveries during development. The hope in keeping these files around is that they may provide some valuable insight during future development.

In presenting the Snail Language Support extension (in posters, presentations, and this paper), we found the Mermaid diagramming tool quite helpful. Mermaid is a tool that allows developers to create high quality diagrams using Markdown-inspired text \cite{MermaidJS_2023a}. Mermaid diagrams can be developed in Mermaid's online editor, using third-party plug ins, or by using Mermaid's API in JavaScript files \cite{MermaidJS_2023b}. 
...
We also made use of a web-based diagramming tool called draw.io. Draw.io is a free, open source diagramming tool that runs in a web browser \cite{drawio_2023}. Draw.io provides a simple and easy-to-use user interface to allow developers to create high quality, customisable diagrams quickly. We use the third-party Draw.io Integration extension for VS Code to create and export draw.io diagrams into \lstinline{.png} files \cite{Dieterichs_2023}. This allows us to easily create diagrams such as those seen in figure \ref{fig:directory-structure} and figure \ref{fig:lsp}.

Despite software documentation being one of the central goals of Snail Language Support, we found it difficult to generate high quality documentation. First, it was difficult to motivate ourselves to generate documentation during development. Instead of documenting steps taken while troubleshooting, we found it easier to save time in the short term to continue developing the extension. Ultimately, this would result in longer development time as we would have to repeat unsuccessful troubleshooting steps when encountering similar issues in the future.

We also didn't realize how difficult making good documentation actually is. As frequent users of documentation, we know what lackluster documentation looks like when we see it. However, when trying to make documentation for Snail Language Support, we didn't realize the attention to detail and thoroughness that is required to generate high quality documentation for a piece of software. After developing Snail Language Support, we have a newfound appreciation for high quality documentation. 
